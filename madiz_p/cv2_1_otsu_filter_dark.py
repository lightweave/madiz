...
img_without_trecks = Image.open("test_HQ_shutt_sharp.png").convert("L") #сюда вставляем изображение без треков
width, height = img_with_trecks.size

all_brights = np.asarray(img_with_trecks , dtype='uint8').copy()
all_brights2=np.asarray(img_without_trecks , dtype='uint8').copy()

threshold = threshold_multiotsu(image=all_brights,classes=3)
const_br1 = threshold.max() + 100
const_br2 = threshold.min() + 50
